...
const priorityLabels: Record<string, { label: string; description: string }> = {
  schools: { label: "Schools", description: "Quality of school districts" },
  commute: { label: "Commute", description: "Transit & highway access to Manhattan" },
  affordability: { label: "Affordability", description: "Lower price point & taxes" },
  safety: { label: "Safety", description: "Low crime & family-friendly" },
  lifestyle: { label: "Lifestyle", description: "Dining, shopping & walkability" },
  investment: { label: "Investment", description: "Appreciation potential & ROI" },
};
...
export default function MoveScoreQuiz() {
  ...
  const [priorities, setPriorities] = useState<Record<string, number>>({
    schools: 5, commute: 5, affordability: 5, safety: 5, lifestyle: 5, investment: 5,
  });
  ...
  const results = useMemo(() => {
    ...
    return filtered.map((n) => ({ neighborhood: n, score: calculateMoveScore(priorities, n) }))
      .sort((a, b) => b.score - a.score);
  }, [selectedRegion, priorities]);

  const displayedResults = showAll ? results : results.slice(0, 5);
  const handlePriority = (key: string, value: number) => setPriorities((prev) => ({ ...prev, [key]: value }));
      ...
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="font-body text-bh-slate max-w-xl mx-auto">
            Tell us your priorities. We&apos;ll rank every neighborhood in Robin&apos;s market based on what matters most to you.
          </p>
        </div>
        ...
        {step === "welcome" && (
          <div className="text-center animate-fade-up">
            <div className="bg-white rounded-xl shadow-lg p-12 max-w-2xl mx-auto border border-bh-gold/10">
              <div className="w-16 h-16 rounded-full bg-robin-navy flex items-center justify-center mx-auto mb-6">
                <span className="text-bh-gold font-mono text-lg font-bold">MS</span>
              </div>
              <h3 className="font-display text-2xl font-bold text-robin-navy mb-3">
                Find Your Perfect Neighborhood
              </h3>
              <p className="font-body text-bh-slate mb-8">
                60-second quiz matching your lifestyle priorities to real neighborhood data across Queens, Nassau County &amp; Western Suffolk. No signup required.
              </p>
        ...
        {step === "region" && (
          <div className="animate-fade-up">
            <div className="bg-white rounded-xl shadow-lg p-10 max-w-2xl mx-auto border border-bh-gold/10">
              <h3 className="font-display text-xl font-bold text-robin-navy mb-2">Where are you looking?</h3>
              <p className="font-body text-bh-slate text-sm mb-8">Select a region or search all markets.</p>
              ...
              <div className="flex gap-3">
                <button onClick={() => setStep("welcome")}
                  className="border border-gray-300 text-bh-slate px-6 py-3 font-body font-medium text-sm rounded-sm hover:bg-gray-50 transition-all">Back</button>
                <button onClick={() => setStep("priorities")}
        ...
        {step === "priorities" && (
          <div className="animate-fade-up">
            <div className="bg-white rounded-xl shadow-lg p-10 max-w-2xl mx-auto border border-bh-gold/10">
              <h3 className="font-display text-xl font-bold text-robin-navy mb-2">What matters most to you?</h3>
              <p className="font-body text-bh-slate text-sm mb-8">Drag each slider to reflect your priorities. Higher = more important.</p>
              <div className="space-y-6 mb-10">
                {Object.entries(priorityLabels).map(([key, { label, description }]) => (
                  <div key={key}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-body font-semibold text-bh-charcoal text-sm">{label}</span>
                      <span className="font-mono font-bold text-bh-burgundy text-sm">{priorities[key]}/10</span>
                    </div>
                    <input type="range" min={0} max={10} value={priorities[key]}
                      onChange={(e) => handlePriority(key, parseInt(e.target.value))} className="w-full" />
          ...
          <div className="animate-fade-up">
            {results.length > 0 && (
              <div className="bg-robin-navy rounded-xl p-8 md:p-12 mb-8 text-center relative overflow-hidden">
                ...
                <span className="text-bh-gold text-xs uppercase tracking-[0.3em] font-body font-semibold">Your #1 Match</span>
                <h3 className="font-display text-3xl md:text-4xl font-bold text-white mt-3 mb-2">{results[0].neighborhood.name}</h3>
                ...
                <div className="relative inline-flex items-center justify-center mb-6">
                  <svg width="120" height="120" viewBox="0 0 120 120">
                    ...
                    <circle cx="60" cy="60" r="50" fill="none" stroke={getScoreRingColor(results[0].score)}
                      strokeWidth="8" strokeLinecap="round"
                      strokeDasharray={`${(results[0].score / 100) * 314} 314`}
                      transform="rotate(-90 60 60)" style={{ transition: "stroke-dasharray 1s ease-out" }} />
                  </svg>
                  <span className="absolute font-display text-3xl font-bold text-white">{results[0].score}</span>
                </div>
                <p className="text-white/70 font-body max-w-lg mx-auto text-sm leading-relaxed">{results[0].neighborhood.description}</p>
                <div className="flex flex-wrap justify-center gap-2 mt-6">
                  {results[0].neighborhood.highlights.map((h) => (
                    <span key={h} className="bg-white/10 text-white/80 px-3 py-1 rounded-full text-xs font-body">{h}</span>
                  ))}
                </div>
                <div className="mt-8 flex flex-wrap justify-center gap-8 text-center">
                  <div>
                    <div className="text-bh-gold font-display text-xl font-bold">{formatPrice(results[0].neighborhood.medianPrice)}</div>
                    <div className="text-white/40 text-xs font-body mt-1">Median Price</div>
                  </div>
                  <div>
                    <div className="text-bh-gold font-display text-xl font-bold">{formatPrice(results[0].neighborhood.avgPropertyTax)}</div>
                ...
                <div key={r.neighborhood.name}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-display font-bold text-robin-navy">{r.neighborhood.name}</span>
                      <span className="text-xs text-bh-slate/50 font-body">{r.neighborhood.region}</span>
                    </div>
                  ...
                  <div className={`font-display text-2xl font-bold ${getScoreColor(r.score)}`}>{r.score}</div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-10 bg-white rounded-xl border border-bh-gold/20 p-8 text-center">
              <h4 className="font-display text-xl font-bold text-robin-navy mb-2">Want to see these neighborhoods in person?</h4>
              <p className="font-body text-bh-slate text-sm mb-6">Robin can set up private tours and pull live market data for your top matches.</p>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                ...
                <button onClick={() => { setStep("welcome"); setShowAll(false); }}
