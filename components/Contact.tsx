...
export default function Contact() {
  const [formData, setFormData] = useState({ name: "", email: "", phone: "", type: "buyer", message: "" });
  ...
  return (
    <div className="py-24 relative overflow-hidden"
      style={{ backgroundImage: "url('/victorlazarovubFLRP4c1gunsplash.jpg')", backgroundSize: "cover", backgroundPosition: "center" }}>
      <div className="absolute inset-0 bg-robin-navy/85 backdrop-blur-sm" />
      <div className="relative z-10 max-w-3xl mx-auto px-6">
        <div className="text-center mb-12">
          <span className="text-bh-gold text-xs uppercase tracking-[0.3em] font-body font-semibold">Get in Touch</span>
          <h2 className="font-display text-4xl md:text-5xl font-bold text-white mt-3 mb-4">Start the Conversation</h2>
        ...
        {submitted ? (
          <div className="bg-white/10 backdrop-blur-lg rounded-xl p-12 text-center border border-white/10">
            <div className="w-16 h-16 rounded-full bg-emerald-500/20 flex items-center justify-center mx-auto mb-4">
              <span className="text-emerald-400 text-2xl font-bold">&check;</span>
            </div>
            <h3 className="font-display text-2xl font-bold text-white mb-2">Message Sent</h3>
            <p className="font-body text-white/70">Robin will get back to you within 24 hours. For urgent inquiries, call directly.</p>
          ...
          <form onSubmit={handleSubmit}
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <div>
                <label className="block text-white/70 text-xs font-body font-medium uppercase tracking-wider mb-2">Name</label>
              ...
              <div>
                <label className="block text-white/70 text-xs font-body font-medium uppercase tracking-wider mb-2">Phone</label>
            ...
            <div>
              <label className="block text-white/70 text-xs font-body font-medium uppercase tracking-wider mb-2">Email</label>
              <input type="email" required value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white font-body placeholder:text-white/30 focus:outline-none focus:border-bh-gold transition-colors"
                placeholder="you@email.com" />
            </div>
            <div>
              <label className="block text-white/70 text-xs font-body font-medium uppercase tracking-wider mb-2">I&apos;m a...</label>
              <div className="grid grid-cols-3 gap-3">
                {[{ value: "buyer", label: "Buyer" }, { value: "seller", label: "Seller" }, { value: "both", label: "Both" }].map((option) => (
                  <button key={option.value} type="button" onClick={() => setFormData({ ...formData, type: option.value })}
            ...
            <div>
              <label className="block text-white/70 text-xs font-body font-medium uppercase tracking-wider mb-2">Message</label>
