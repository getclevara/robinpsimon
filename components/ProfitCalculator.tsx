...
export default function ProfitCalculator() {
  ...
  const results = useMemo(
    () => calculateNetProceeds(salePrice, mortgageBalance, commission / 100),
    [salePrice, mortgageBalance, commission]
  );
  ...
  // Where equity could take you
  const equityDestinations = useMemo(() => {
    if (results.netProceeds <= 0) return [];
    const downPayment20 = results.netProceeds;
    const maxPurchase = downPayment20 / 0.2;

    return neighborhoods
      .filter((n) => n.medianPrice <= maxPurchase * 1.1)
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          ...
          <div className="bg-bh-cream rounded-xl p-8 border border-bh-gold/10">
            <h3 className="font-display text-xl font-bold text-robin-navy mb-6">
            ...
            <div className="mb-6">
              <label className="flex items-center justify-between mb-2">
                ...
                <span className="font-display font-bold text-bh-burgundy">
              ...
              <input
                ...
                className="w-full"
              />
              <div className="flex justify-between text-xs text-bh-slate/50 font-body mt-1">
                <span>$200K</span>
                <span>$3M</span>
              </div>
            </div>
            ...
            <div className="mb-8">
              <label className="flex items-center justify-between mb-2">
                ...
                <span className="font-display font-bold text-bh-burgundy">{commission}%</span>
              </label>
              <input
                type="range"
                min={3}
            ...
            {showResults ? (
              <div className="animate-fade-up">
                ...
                <div className="bg-robin-navy rounded-xl p-8 text-center mb-6 relative overflow-hidden">
                  ...
                  <div
                    className={`font-display text-4xl md:text-5xl font-bold mt-3 ${
                      results.netProceeds >= 0 ? "text-emerald-400" : "text-red-400"
                    }`}
                  >
                    {formatPrice(results.netProceeds)}
                  </div>
                  <p className="text-white/40 font-body text-xs mt-2">
                    After all closing costs &amp; fees
                  </p>
                </div>

                {/* Cost breakdown */}
                <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-3">
                  <h4 className="font-display font-bold text-robin-navy mb-4">
                ...
                {equityDestinations.length > 0 && (
                  <div className="mt-6 bg-bh-cream rounded-xl p-6 border border-bh-gold/10">
                    <h4 className="font-display font-bold text-robin-navy mb-1">
                      Where Your Equity Could Take You
                    </h4>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {equityDestinations.map((n) => (
                        ...
                        >
                          <span className="font-display font-bold text-robin-navy text-sm">
                            {n.name}
                          </span>
                          <div className="text-xs text-bh-slate font-body mt-0.5">
                            {formatPrice(n.medianPrice)} median
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <div className="h-full flex items-center justify-center">
                <div className="text-center p-12">
                  ...
                  <h3 className="font-display text-xl font-bold text-robin-navy/30 mb-2">
